# Classic Platform
![alt text](image-cp-archi.png)
# Doc: Explanation of Application Interface of AD/ADAS vehicle motion control
![alt text](image-ADAS-scope.png)
![alt text](image-func-archi-of-vehicle-ctrl.png)
# Doc: Explanation of Application Interfaces of the HMI, Multimedia and Telematics Domain
![alt text](image-hmi-dep.png)
### Example: AUTOSAR PDC application
![alt text](image-parking-distance-contrl.png)
# Doc: Explanation of Application Interfaces of the Chassis Domain
![alt text](image-car-center-of-gravity.png)
![alt text](image-chassis-domain.png)
![alt text](image-steer.png)
# Adaptive Platform
![alt text](image-adaptive-platform.png)
# Doc: Explanation of Sensor Interfaces
![alt text](image-ad-sensors-conn-to-ecu.png)
# Doc: Explanation of Adaptive Platform Software Architecture
## Example
![alt text](image-adaptive-platform-sotware-update-interface.png)
## Example 
![alt text](image-interface-file-storage.png)